...
public class SupplierPage extends JFrame {
    ...
    private JTextField tfID, tfName, tfContact, tfSearchID;
    ...
    private static final String[] COLS = {"ID","Supplier Name","Contact Information"};
    ...
    public SupplierPage() {
        ...
        JPanel left = new JPanel(null) {
            ...
            @Override public Dimension getPreferredSize() { return new Dimension(230, 0); }
        };
        ...
        right.setPreferredSize(new Dimension(728, 640));
        ...
        table.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting() && table.getSelectedRow() >= 0) {
                ...
                tfID.setText(model.getValueAt(r,0)+"");
                tfName.setText(model.getValueAt(r,1)+"");
                tfContact.setText(model.getValueAt(r,2)+"");
                tfSearchID.setText(model.getValueAt(r,0)+"");
            }
        });
    }
    ...
    private void paintLeft(Graphics2D g2, int w, int h) {
        ...
        g2.setPaint(new RadialGradientPaint(cx,cy,120,new float[]{0f,1f},new Color[]{new Color(188,152,90,22),new Color(0,0,0,0)})); g2.fillRect(0,0,w,h);
        ...
        String sub="Supplier Management"; g2.drawString(sub, cx-fm.stringWidth(sub)/2, ry+17);

        // Count badge
        int count=model==null?0:model.getRowCount();
        int bx=cx-55,by=ry+34;
        g2.setColor(new Color(188,152,90,25)); g2.fillRoundRect(bx,by,110,42,6,6);
        g2.setColor(new Color(188,152,90,60)); g2.setStroke(new BasicStroke(0.7f)); g2.drawRoundRect(bx,by,110,42,6,6);
        ...
        String cl=count==1?"supplier":"suppliers"; g2.drawString(cl, cx-fm.stringWidth(cl)/2, by+39);

        // Tips
        int ty=by+60;
        String[]tips={"Select a row to fill form","Edit & click Update to save","Delete removes permanently"};
        for(int i=0;i<tips.length;i++){
            g2.setColor(new Color(75,70,65)); g2.setFont(new Font("Georgia",Font.ITALIC,9)); fm=g2.getFontMetrics();
            g2.drawString("· "+tips[i], bx, ty+i*14);
        }
    ...
    private JPanel buildRight() {
        ...
        JLabel tl=new JLabel("Supplier Management"); tl.setFont(new Font("Georgia",Font.BOLD,20)); tl.setForeground(C_CHARCOAL);
        JLabel sl=new JLabel("Manage dental supply partners and contact information"); sl.setFont(new Font("Georgia",Font.ITALIC,12)); sl.setForeground(C_GOLD);
        ...
        JLabel slbl=new JLabel("Search ID:"); slbl.setFont(new Font("Georgia",Font.BOLD,12)); slbl.setForeground(C_CHARCOAL);
        tfSearchID=new JTextField(8); styleField(tfSearchID);
        JButton sb=mkBtn("Search",false,null); sb.addActionListener(e->searchSupplierByID()); tfSearchID.addActionListener(e->searchSupplierByID());
        sr.add(slbl); sr.add(tfSearchID); sr.add(sb);
        top.add(sr,BorderLayout.EAST);
        p.add(top,BorderLayout.NORTH);

        // Center: form + table 50/50
    ...
    private JPanel buildFormCard() {
        ...
        GridBagConstraints g=new GridBagConstraints(); g.insets=new Insets(8,10,8,10); g.anchor=GridBagConstraints.WEST;

        tfID=makeField(); tfName=makeField(); tfContact=makeField();

        String[]labels={"Supplier ID","Supplier Name","Contact Info"};
        JTextField[]fields={tfID,tfName,tfContact};
        for(int i=0;i<labels.length;i++){
            g.gridx=0;g.gridy=i; JLabel l=new JLabel(labels[i]); l.setFont(new Font("Georgia",Font.BOLD,13)); l.setForeground(C_CHARCOAL); card.add(l,g);
        ...
        g.gridx=0;g.gridy=3;g.gridwidth=2;g.weighty=1.0;g.fill=GridBagConstraints.VERTICAL;
    ...
    private JPanel buildTableCard() {
        ...
        table=new JTable(model); table.setFont(new Font("Georgia",Font.PLAIN,12)); table.setRowHeight(30); table.setShowGrid(false);
        ...
        table.setDefaultRenderer(Object.class,new DefaultTableCellRenderer(){
            @Override public Component getTableCellRendererComponent(JTable t,Object v,boolean sel,boolean foc,int row,int col){
                super.getTableCellRendererComponent(t,v,sel,foc,row,col); setBorder(new EmptyBorder(0,8,0,6));
        ...
        int[]widths={55,200,270}; for(int i=0;i<widths.length;i++)table.getColumnModel().getColumn(i).setPreferredWidth(widths[i]);
    ...
    private JPanel makeCard(){
        ...
        c.setOpaque(false); c.setBorder(new EmptyBorder(16,18,16,18)); return c;
    }
    ...
    private JPanel buildButtonBar(){
        ...
        JButton bAdd=mkBtn("+ Add",true,null),bUpd=mkBtn("Update",false,null),bDel=mkBtn("Delete",false,C_DANGER),bClr=mkBtn("Clear",false,null);
        bAdd.addActionListener(e->addSupplier()); bUpd.addActionListener(e->updateSupplier()); bDel.addActionListener(e->deleteSupplier()); bClr.addActionListener(e->clearForm());
        ...
        JButton back=mkBtn("← Back",false,null); back.addActionListener(e->{dispose();try{new SecretaryMenu().setVisible(true);}catch(Exception ignored){}});
    ...
    private JTextField makeField(){JTextField f=new JTextField();f.setPreferredSize(new Dimension(155,30));styleField(f);return f;}
    ...
    private void loadTableData(){
        model.setRowCount(0);
        for(Supplier s:supplierManager.getAllSuppliers())
            model.addRow(new Object[]{s.getSupplierID(),s.getSupplierName(),s.getContactInformation()});
        repaint();
    }
    ...
    private void addSupplier(){
        try{
            ...
            supplierManager.addSupplier(id,tfName.getText().trim(),tfContact.getText().trim());
            loadTableData(); clearForm(); JOptionPane.showMessageDialog(this,"Supplier added.");
    ...
    private void updateSupplier(){
        try{
            int id=Integer.parseInt(tfID.getText().trim());
            supplierManager.updateSupplier(id,tfName.getText().trim(),tfContact.getText().trim());
            loadTableData(); clearForm(); JOptionPane.showMessageDialog(this,"Supplier updated.");
        }catch(Exception ex){JOptionPane.showMessageDialog(this,ex.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);}
    }
    ...
    private void deleteSupplier(){
        try{
            int id=Integer.parseInt(tfID.getText().trim());
            int ok=JOptionPane.showConfirmDialog(this,"Delete supplier ID "+id+"?","Confirm",JOptionPane.YES_NO_OPTION);
            if(ok==JOptionPane.YES_OPTION){supplierManager.removeSupplier(id);loadTableData();clearForm();JOptionPane.showMessageDialog(this,"Supplier deleted.");}
        }catch(Exception ex){JOptionPane.showMessageDialog(this,ex.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);}
    }
    ...
    private void searchSupplierByID(){
        try{
            ...
            if(s!=null){tfID.setText(""+s.getSupplierID());tfName.setText(s.getSupplierName());tfContact.setText(s.getContactInformation());}
            else JOptionPane.showMessageDialog(this,"Supplier not found.");
        }catch(NumberFormatException ex){JOptionPane.showMessageDialog(this,"Invalid ID.","Error",JOptionPane.ERROR_MESSAGE);}
    }

    private void clearForm(){tfID.setText("");tfName.setText("");tfContact.setText("");tfSearchID.setText("");}

    public static void main(String[]args){SwingUtilities.invokeLater(()->new SupplierPage().setVisible(true));}
}